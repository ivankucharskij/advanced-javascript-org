...
1. `[2,3], then [1,"a","b",4]`
   - correct: true
   - feedback: Correct. The snippet normalizes splice arguments, removes a segment, inserts new items, and returns deleted values, so the output is `[2,3], then [1,"a","b",4]`.
...
1. `[3,4], then [1,2]`
   - correct: true
   - feedback: Correct. This follows the same implementation, so the output is `[3,4], then [1,2]`.
