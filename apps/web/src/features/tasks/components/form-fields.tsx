...
export default function FormFields({
  ...
  defaultValues?: GetTasks200DataItem;
  addDataCallback?: (data: PostTasksBody) => Promise<unknown>;
  editDataCallback?: (data: PatchTasksIdBody) => Promise<unknown>;
  loading: boolean;
}) {
  const {
    register,
  ...
  const onSubmit = async (data: AddTaskFormFields | EditTaskFormFields) => {
    if (addDataCallback) {
      await addDataCallback({
        ...
        priority: data.priority as GetTasks200DataItemPriority,
        status: data.status as GetTasks200DataItemStatus,
      });
    }
