...
- `/snippet-test` also has temporary admin seed buttons. Seed snippets first from `snippets.ts`, then seed challenges from `seed-challenges.ts`; challenge seeding depends on persisted `snippetId` values.
